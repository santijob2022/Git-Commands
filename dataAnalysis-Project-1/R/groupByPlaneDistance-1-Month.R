...
proof2 <-
  ...
  select(rideable_type,started_at,start_lat,start_lng,end_lat,end_lng,
         member_casual,ride_duration_hours) %>% 
  drop_na(start_lat,end_lat) %>% 
  mutate(plane_distance = sqrt((start_lat-end_lat)^2+(start_lng-end_lng)^2)) %>% 
  select(rideable_type, started_at, member_casual, ride_duration_hours,plane_distance) %>% 
  ...
  group_by(Intervals, member_casual) %>% 
  summarise(total_by_interval = n()) %>%
  group_by(Intervals,member_casual) %>% 
  mutate(percentage = case_when(
      member_casual == "casual" ~ as.numeric((total_by_interval*100)/total_by_memberType[1,2]),
      member_casual == "member" ~ as.numeric((total_by_interval*100)/total_by_memberType[2,2])
    )
    ) 

View(proof2)

# Here we separate the graph from the dataFrame which we names "proof2"
proof2 %>% 
  filter(Intervals != -1) %>% 
